...
// Sample words list for the game
const words = [
  "aahed",
  "aalii",
  "aapas",
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM fully loaded - attaching direct event listeners');
    ...
    scoreTabs.forEach(function(tab) {
        tab.addEventListener('click', function(e) {
            ...
            // Load scores for the selected period
    ...
    // Initialize keyboard interactions - Moved this here to prevent duplicate event listeners
    initializeKeyboard();
...
function shareResult() {
    console.log('Sharing game results');
    ...
    // Create share text
    const shareText = `Wordle ${attempts}/${maxAttempts}\n\n${emojiGrid}\nPlay at: ${window.location.href}`;
...
async function loadScores(period = 'today') {
    try {
        console.log(`Loading scores for period: ${period}`);
        ...
        if (!data.success) {
            scoreList.innerHTML = '<div class="score-item">Error loading scores</div>';
            return;
        }
        
        if (data.scores.length === 0) {
            scoreList.innerHTML = '<div class="score-item empty-list">No scores available for this period</div>';
            return;
        }
        ...
        data.scores.forEach((score, index) => {
            ...
            details.classList.add('score-details');
            details.textContent = `${score.attempts}/6 | ${score.time}s`;
...
// Notification function - used to show temporary messages to the user
...
async function authenticateUser(username, password, isRegistration) {
    try {
        console.log(`Authenticating user ${username}, isRegistration: ${isRegistration}`);
...
function showHighscores() {
    ...
    // Load scores (placeholder for now)
    scoreList.innerHTML = '<div class="score-item">Loading scores...</div>';
}

// Hide high scores modal
function hideHighscores() {
    console.log('Hiding highscores');
    highscoresSection.classList.add('hidden');
}

// Show how to play modal
function showHowToPlay() {
    console.log('Showing how to play modal');
    howToPlayModal.classList.remove('hidden');
}

// Hide how to play modal
function hideHowToPlay() {
    console.log('Hiding how to play modal');
...
async function handleLogin() {
    ...
    const response = await authenticateUser(username, password, false);
...
async function handleRegister() {
    ...
    const response = await authenticateUser(username, password, true);
...
// Animate the tiles after a guess
...
function saveScore(score) {
    ...
    .catch(error => {
        ...
        showNotification('Error saving score. Please check your connection.');
    });
}

// Initialize keyboard interactions - attaches event listeners to the on-screen keyboard buttons
function initializeKeyboard() {
    console.log('Initializing keyboard interactions');
    const keys = document.querySelectorAll('.keyboard-row button');
...
// Load active players for admin panel
async function loadActivePlayers() {
    try {
        ...
        const data = await response.json();
        console.log('Active players response:', data);
        ...
        if (!activePlayersList || !activePlayerCount) {
            return;
        }
        
        activePlayersList.innerHTML = '';
        activePlayerCount.textContent = data.players.length;
        
        if (data.players.length === 0) {
            const emptyItem = document.createElement('div');
            emptyItem.classList.add('admin-item', 'empty-list');
            emptyItem.textContent = 'No active players';
        ...
        data.players.forEach(player => {
            ...
            timeSpan.classList.add('admin-time');
            const time = new Date(player.last_activity).toLocaleTimeString();
            timeSpan.textContent = `Last active: ${time}`;
...
// Load all scores for admin panel
async function loadAllScores() {
    try {
        console.log('Loading all scores for admin');
        const response = await fetch(`${API_ENDPOINTS.SCORES}?admin=true`, {
            method: 'GET',
            credentials: 'include' // Include cookies for session
        });
        ...
        data.scores.forEach(score => {
            ...
            detailsSpan.textContent = `${score.attempts}/6 | ${score.time}s | ${score.word}`;
            ...
            deleteBtn.classList.add('admin-delete');
            deleteBtn.dataset.scoreId = score.score_id;
            deleteBtn.innerHTML = '<i class="fas fa-trash"></i>';
            
            // Add click handler directly to ensure it's properly attached
            deleteBtn.addEventListener('click', function() {
                if (confirm(`Are you sure you want to delete this score from ${score.username}?`)) {
                    deleteScore(score.score_id);
                }
            });
...
async function deleteScore(scoreId) {
    try {
        ...
        showNotification('Deleting score...');
        
        // Log the data being sent for debugging
        const requestData = { score_id: scoreId };
        ...
        if (data.success) {
            ...
            // Reload scores list to reflect the change
            loadAllScores();
        } else {
            console.error('Error deleting score:', data.message);
            showNotification(`Error deleting score: ${data.message || 'Unknown error'}`);
        }
    } catch (error) {
        console.error('Error deleting score:', error);
        showNotification('Network error while deleting score');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add event handlers for login/register tabs
    if (loginTab) {
        console.log('Adding login tab event handler');
    ...
    if (registerTab) {
        console.log('Adding register tab event handler');
    ...
    // Add event handlers for login/register buttons
    if (loginBtn) {
        console.log('Adding login button event handler');
    ...
    if (registerBtn) {
        console.log('Adding register button event handler');
    ...
    if (logoutBtn) {
        console.log('Adding logout button event handler');
    ...
    // Add event handler for "View High Scores" button
    if (showScoresBtn) {
        console.log('Adding show scores button event handler');
        showScoresBtn.addEventListener('click', (e) => {
            e.preventDefault();
            showHighscores();
        });
    }
    
    // Add event handler for "Close Scores" button
    if (closeScoresBtn) {
        console.log('Adding close scores button event handler');
        closeScoresBtn.addEventListener('click', (e) => {
            e.preventDefault();
            hideHighscores();
        });
    }
    
    // Add event handler for "How to Play" button
    if (howToPlayBtn) {
        console.log('Adding how to play button event handler');
    ...
    // Add event handler for "Close How to Play" button
    if (closeHowToPlayBtn) {
        console.log('Adding close how to play button event handler');
        closeHowToPlayBtn.addEventListener('click', (e) => {
            e.preventDefault();
            hideHowToPlay();
        });
    }
    
    // Add event handler for clicking outside the how-to-play modal to close it
    ...
    // Add event handler for the new game button
    if (newGameBtn) {
        console.log('Adding new game button event handler');
    ...
    if (adminButton) {
        console.log('Adding admin button event handler');
    ...
    if (closeAdminBtn) {
        console.log('Adding close admin button event handler');
    ...
    // REMOVED: Duplicate keyboard event listeners
    // Already handled by initializeKeyboard() in the main DOMContentLoaded handler
    ...
    adminTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            document.getElementById(`admin-${tabContent}`).classList.remove('hidden');
            
            // Load data if needed
            if (tabContent === 'active-players') {
                loadActivePlayers();
            } else if (tabContent === 'all-scores') {
                loadAllScores();
            }
        });
    });
    ...
    // Set up an interval to refresh active players list in admin panel
    const adminRefreshInterval = setInterval(() => {
        if (isAdmin && !adminPanel.classList.contains('hidden') && document.querySelector('.admin-tab.active').getAttribute('data-tab') === 'active-players') {
            loadActivePlayers();
        }
    }, 5000);
});